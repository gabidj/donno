...
```bash
composer dev           # Start development server (Vite + Laravel)
composer test          # Run all tests and linting
composer test:lint     # Check PHP style with Pint
npm run dev           # Start Vite dev server (frontend only)
npm run build         # Build assets for production
npm run lint          # Lint and format JavaScript/Vue
npm run format        # Format JavaScript/Vue files
...
This project uses Laravel, Pest tests, and Pint linting to create a feedback loop for AI-generated code:
...
- **Pest tests are executable specifications**: Test failures tell you exactly what's broken (file:line, expected vs actual)
- **Type errors are precise**: PHP reveals type mismatches, undefined properties, and method calls
- **Pint catches style issues**: Ensures consistent formatting across the codebase
- **Quick feedback loop**: Tests run in seconds, enabling frequent iteration

### Reading error output:
- **Test failures**: Show the test name, assertion, and diff
- **Lint errors**: Include file, line number, and description
- **PHP errors**: Include file:line and detailed error message
...
- Place tests in `tests/Unit/` for unit tests (models, services)
- Place tests in `tests/Feature/` for integration tests (routes, controllers)
- Use `test()` or `it()` blocks from Pest
- Use descriptive test names: `test('creates a user with valid email')`
...
- **Integration**: Inertia.js (server-driven SPA)
- **Styling**: Tailwind CSS 4
- **Database**: SQLite (dev), PostgreSQL (production)
- **Testing**: Pest (PHP) + Vitest (JavaScript)
- **Linting**: Pint (PHP), ESLint + Prettier (JavaScript)
...
├── Http/
│   ├── Controllers/ # Request handlers (thin, dispatch to services)
...
└── Feature/         # Integration tests (routes, controllers)
```

## Service Architecture

Features are organized using the Service pattern. Each service owns its entire stack: business logic, database operations, and error handling. Services are reusable across controllers, jobs, and commands.

**Why services:**
- **Reusability**: Services can be called from controllers, jobs, commands, or other services
- **Testability**: Services are pure PHP classes easy to unit test
- **Separation of Concerns**: Business logic lives in services, not controllers
...
Controllers are thin — they validate input (via Form Requests) and delegate to services:
...
- Leverage Eloquent scopes for queries
...
Components use the Composition API and are located in `resources/js/components/`:

**Component conventions:**
- Use `<script setup>` for simpler components
- Use descriptive prop types with TypeScript
- Emit events with specific type information
...
Pages are Vue 3 components in `resources/js/pages/` that receive server-rendered props:
...
Migrations are in `database/migrations/`. Always include reversible `down()` methods:
...
- Pint linter enforces style automatically
...
- TypeScript by default (`.ts` and `.vue` with `<script setup lang="ts">`)

## Rules That Will Fail Checks

**PHP Errors (Pint + Pest):**
- Unused imports or variables
- Missing type hints on methods
- Not following PSR-12 style
- Undefined properties or methods
- Test assertions that don't match expected values
...
For structured logging in actions:
```php
class CreateUser
{
