...
| Service | URL | Purpose |
|---------|-----|---------|
| Frontend | `http://localhost` | Vue dashboard (Vite dev server) |
| Backend | `http://localhost:8000` | Laravel API (`php artisan serve`) |
| Health Check | `http://localhost:8000/health` | Server status (Laravel) |
...
4. **Use `--headed` mode** for debugging: `agent-browser open http://localhost --headed` (frontend) or `http://localhost:8000` (backend)
