...
|------------|------------|
| PHP bugs | Grep error messages, read full context, git blame, check app/Services/, check Models/relationships, **run tests with edge inputs** |
| Laravel runtime issues | Check .env/config, Service initialization, database migrations, error handling in Controllers |
| Vue/Inertia integration | Trace data flow from Controller→Inertia→Component, check props, verify state management |
| Regressions | `git log --oneline -20` app/, `git diff HEAD~10` for affected files, check migration history |
