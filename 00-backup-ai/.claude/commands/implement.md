...
| Failure | Action |
|---------|--------|
| Lint fails (Pint) | Fix PHP style issues, re-run `composer test:lint` |
| Tests fail | Fix implementation or test, re-run `composer test` |
| Frontend lint fails | Run `npm run lint` for issues, fix and re-run |
| PHP validation fails | Check error output, fix and re-run |
